/*
  ...
  # Description
    ...
    G の極大マッチング M を貪欲的に求めて M の端点の集合を頂点被覆 S として出力する．

    Prop. G の最小頂点被覆 S* に対して，|S| <= 2 |S*|
    証明：
    　任意の辺 {u, v} \in M に対して，S* は頂点被覆なので u \in S* または v \in S* が成り立つ．
    　また，M はマッチングなので異なる M の2辺を被覆する S* の頂点は異なる．
    　したがって，|M| <= |S*| が成り立つ．このとき，2|M| = |S| であるから，|S| <= 2|S*| となる．
