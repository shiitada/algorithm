/*
  Eulerian Digraph (Hierholzer Algorithm)
  オイラー有向グラフのハイヤーホルザのアルゴリズム
  =======================================

  # Problem
    Input: 多重有向グラフ G = (V, A)
    Output: G がオイラーグラフか準オイラーグラフかを判定して，存在するならばオイラー道かオイラー閉路を求める

    Def. オイラー路 (Eulerian path)
      G のオイラー道とは，G のすべての弧をちょうど1回通る有向道

    Def. 準オイラーグラフ (semi-Eulerian graph)
      G にオイラー道が存在するとき，G をオイラーグラフと呼ぶ
  ...
  # Usage
    ...
    - g.path: g の オイラー道かオイラー閉路の訪れた頂点の列
  ...
  # Description
    ...
    Thm. 準オイラーグラフ
      ...
      出次数が等しく，s からすべての頂点への有向道が存在することである．

    初めに，入次数と出次数の条件を調べる．次に，Hierholzer のアルゴリズムで部分オイラー道または
