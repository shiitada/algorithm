/*
  ...
  # Description
    頂点 v の深さを d(v) としたとき，v から根へ d(v) 回祖先を辿ることによって根に到達することができる．
    また，d(v) >= d(u) とすると，初めに v から根へ d(v) - d(u) 回祖先を辿り，次に u と v を
    同時に1回ずつ共通の頂点に到達するまで祖先を辿ることによって最小共通祖先に到達することができる．
    ...
    上の説明の， v から根へ d(v) - d(u) 回祖先を辿ることは，冪乗法のようにすると O(log n) 回で到達できる．
    また，最小共通祖先からは何回祖先を辿っても共通の頂点となるので，u と v から同時に1回ずつ上へ辿る方法も，
